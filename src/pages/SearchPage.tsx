...
export default function SearchPage() {
  ...
  const runSearch = useMemo(() => async () => {
    ...
    if (cat !== "all") query = query.eq("category_key", cat);
  ...
  const clear = () => {
    setQ(""); setCat("all"); setTag(""); setSort("newest");
    setParams({});
  };
      ...
      <section className="container py-6 md:py-10">
        ...
        <form onSubmit={onSubmit} className="bg-card border border-border rounded-2xl p-4 md:p-5 space-y-3">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button type="submit" className="px-5 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90">
              بحث
            </button>
            {(q || cat !== "all" || tag) && (
              <button type="button" onClick={clear} className="px-3 py-2 text-sm text-muted-foreground hover:text-foreground">
                إعادة تعيين
              </button>
            )}
          </div>
        ...
        <div className="mt-6">
          <p className="text-sm text-muted-foreground mb-4">
            {loading ? "جارٍ البحث..." : `${results.length} نتيجة`}
          </p>
